...
def restaurantdashbord(request):
    ...
    transactions=Orders.objects.filter(Payment_method="Paid via Razorpay")
    num_transactions=transactions.count()
    return render (request,'adminrestaurant/dashbord.html',{'restaurant':Restaurants_data,'no_restau':no_restauarnt,'totalorders':total_orders,'totalmenu':total_menu,'total_cancelation':no_canceled,'successorders':num_success,'ongoing':num_ongoing,'totalpromo':num_promos,'transactions':num_transactions})
...
def buysellManagerLogin(request):
    if request.method=='POST':
        email=request.POST['email']
        password=request.POST['password']
        admin=BuySellmanager.objects.filter(Email=email,Password=password).first()
        if admin:
            # Save admin ID and name in the session
            request.session['admin_id'] = admin.id
            #request.session['admin_name'] = admin.Buysell_Boss
...
def revenue_calcu(request):
    ...
    if payment_method == 'cash_on_delivery':
        ...
        orders_query = orders_query.filter(Payment_method='Paid via Razorpay')
